...
const getVoteStatusText = (status: string) => {
  switch (status) {
    case 'pending':
      return 'Vote submitted, awaiting verification'
    case 'verified':
      return 'Vote verified, waiting for aggregation'
    case 'aggregated':
      return 'Vote aggregated, waiting for processing'
    case 'processed':
      return 'Vote processed successfully'
    case 'settled':
      return 'Vote settled and finalized'
    case 'error':
      return 'Vote processing failed'
    default:
      ...
      <ThemeProvider theme={theme}>
        <Layout>
          <Box sx={{ width: '100%', maxWidth: 800, mx: 'auto', px: 3, py: 4 }}>
            <Alert severity='error'>{error || 'Process not found'}</Alert>
            <Button
              variant='outlined'
      ...
      <Layout>
        <Box sx={{ width: '100%', maxWidth: 1200, mx: 'auto', px: 3, py: 4 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
            <IconButton onClick={() => navigate('/processes')} sx={{ mr: 2 }} aria-label='Go back to processes'>
              <ArrowBackIcon />
            </IconButton>
            ...
            <Box sx={{ display: 'flex', gap: 4, mb: 4 }}>
                  ...
                  {!wallet || canUserVote === false ? (
                      ...
                      {canUserVote === false && (
                        <Alert severity='error' sx={{ mb: 3 }}>
                          ...
                          <Typography variant='body2'>
                            Your address ({walletAddress}) is not included in the census for this voting process. Try
                            connecting a different wallet.
                          </Typography>
                          <Button variant='outlined' size='small' onClick={resetWallet} sx={{ mt: 1 }}>
                    ...
                    <>
                      <Alert severity='success' sx={{ mb: 3 }}>
                        ...
                        <Typography variant='body2'>Address: {walletAddress}</Typography>
                      </Alert>
                      ...
                      {checkingEligibility && (
                        <Alert severity='info' sx={{ mb: 3 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <Typography variant='body2'>Checking voting eligibility...</Typography>
                          </Box>
                        </Alert>
                      )}
                            ...
                            <Box sx={{ width: '100%', mb: 4 }}>
                              <Stepper activeStep={activeStep}>
                                {VOTE_STEPS.map((label, index) => (
                                  <Step key={label}>
                                    <StepLabel
                                      StepIconComponent={() => renderStepIcon(index < activeStep, index === activeStep)}
                                    >
                                      {label}
                                    </StepLabel>
                                  </Step>
              ...
              <Card sx={{ flex: '1 1 40%' }}>
                <CardContent>
                  ...
                  {!submittedVote ? (
                    <Typography color='text.secondary'>No vote submitted yet</Typography>
                  ) : (
                    <>
                      <Alert severity={getVoteStatusColor(submittedVote.status) as any} sx={{ mb: 3 }}>
                        <Typography variant='subtitle2'>
                          {submittedVote.status.charAt(0).toUpperCase() + submittedVote.status.slice(1)}
                        </Typography>
                        <Typography variant='body2'>{getVoteStatusText(submittedVote.status)}</Typography>
                      </Alert>
                      ...
                      <Box sx={{ mb: 3 }}>
                        ...
                        <LinearProgress
                          variant='determinate'
                          value={
                            submittedVote.status === 'pending'
                              ? 20
                              : submittedVote.status === 'verified'
                                ? 40
                                : submittedVote.status === 'aggregated'
                                  ? 60
                                  : submittedVote.status === 'processed'
                                    ? 80
                                    : submittedVote.status === 'settled'
                                      ? 100
                                      : 0
                          }
                      ...
                      <List dense>
                        {[
                          ...
                          const isCompleted =
                            ['pending', 'verified', 'aggregated', 'processed', 'settled'].indexOf(
                              submittedVote.status
                            ) >= index
          ...
          {/* Results Section */}
          {processData.status === 4 && <ResultsSection processData={processData} questions={questions} />}
